...
async def join_by_token(token: str, guild_id: int, channel_id: int) -> None:
    ...
    # ── Fix: discord.py adds "Bot " prefix to all tokens by default.
    # User (personal) account tokens must be sent as-is without that prefix.
    async def _user_static_login(self, raw_token: str):
        self.token = raw_token          # no "Bot " prefix — this is a user token
